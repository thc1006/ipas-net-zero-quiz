...
def norm(t):
    ...
    **NFKC 是必要的**：這些中文 PDF 用 CJK 相容字 ——
    「理」可能是 U+F9E4 而不是 U+7406，「生態衝擊」用 str.find() 直接找會回 -1。

    **標點必須用 Unicode 類別剝，不可以手列**：
    ...
    **但絕對不可以整片剝 Sm（數學符號）**：× ÷ + − = 都是 Sm，
    而題庫裡有公式題，四個選項的差別**只在運算子** —— 剝掉會塌成同一個字串。

    而這正是我第一次修錯的地方：我以為項目符號是 `So`，就寫「剝 So、保留 Sm」——
...
def _get(url, timeout=60):
    ...
    但那個修法只是**請求**對方不要壓縮，而 **web.archive.org 根本不理**：
...
def quote_status(quote, page_text):
    ...
    **那不是捏造，是拼接。** 而那一題裡藏著一個真的錯答案（B→C），
    ...
    修法**不可以是「連空白一起切」**。中文很少用空白，英文卻是**每個字都用空白分隔** ——
