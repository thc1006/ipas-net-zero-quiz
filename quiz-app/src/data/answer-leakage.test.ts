...
//
// 它**不會**因為 AI 產題的答案是對的就變綠。`verify_verdict: CONFIRMED`
...
describe('答案洩漏：「無腦選最長」能考幾分', () => {
  ...
  it('AI 產題的洩漏程度不准再惡化（釘在現況、比原始比率不比四捨五入值）', () => {
    // 現況 hit/n = 64/100 = 0.64（64.0%）—— 2026-08-27 改寫 ind-034 時，四個選項寫成等長，
    // 正解不再是最長選項（先前 65.0%；再往前 66.3%）。這個上限只降不升。
    // **比較原始比率，不比四捨五入後的顯示值** —— 否則 0.6504→0.6549 這種小幅倒退會被
    // 進位藏成同一個「65.0%」而不變紅（前一版就是這個盲點）。
    const { hit, n } = longestOptionScore(AI);
    expect(hit / n).toBeLessThanOrEqual(0.64);
  });
