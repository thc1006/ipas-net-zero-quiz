...
describe('練習池：每一道數值題都必須被登記（算術題手算、查表題附出處）', () => {
  ...
  const LOOKUP: ReadonlyArray<{ id: string; expect: string; basis: string }> = [
    ...
    { id: 'pool-em-ipas_vocus_mock-015', expect: '2026年，4月30日', basis: '依溫管辦法「修正草案」（尚未發布施行）；4/30 期限本身是現行 §6 明文' },
    ...
    { id: 'pool-aig-ind-012', expect: '約 68%', basis: '能源署統計：2024 年底太陽光電占再生能源裝置容量 67.8%（逐年變動）' },
  ];
