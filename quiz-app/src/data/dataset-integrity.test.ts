...
describe('quality_flags 的一致性', () => {
  ...
  const CORRECTED_BUT_STABLE: ReadonlyArray<{ id: string; why: string }> = [
    ...
    },
    {
      id: 'gist[420]',
      why:
        '**已撤答案（answer=null、排除計分）。** 使用者最終裁定：選項 D「公共電力淨零聯盟」即 COP28 於 ' +
        '2023/12 由 IRENA 與 COP28 主席國發起之真實聯盟 UNEZA，命題依據 328201936.pdf p.55 亦列為 COP28 行動；' +
        '故 D 為真、「何者不是」不可能選 D。另 C「脫碳聯盟」疑為教材原文「脫煤聯盟」(Powering Past Coal) 之誤植。' +
        '無論語意或逐字判讀，答案都不是 D —— 命題瑕疵、撤答案後不會再變',
    },
    {
      id: 'gist[373]',
      why:
    ...
    },
    {
      id: 'gist[80]',
      why:
        '**這一題已被判定沒有正確選項**（answer=null、排除計分）—— 碳費收費辦法 §9 寫的是' +
        '「定期檢討第一項**國內**減量額度扣除收費排放量之**上限及比率**」，' +
        '全條「國外」出現 0 次；而選項 (A) 寫「國內**外**」、選項 (D) 漏掉「上限」。' +
        '四個選項沒有一個等於法條。條文本身不會因時間而變 —— 要變的話是修法，那時會重驗',
    },
