...
describe('quality_flags 的一致性', () => {
  ...
  const CORRECTED_BUT_STABLE: ReadonlyArray<{ id: string; why: string }> = [
    ...
    },
    {
      id: 'gist[340]',
      why:
        '**答案由 B 更正為 C（回報 issue #103）。** 原以「刪去法」判 B，但依 UNFCCC 決議 1/CMA.5 ' +
        '第 28 段一手原文：28(a) 再生能源三倍＋能源效率加倍、28(d) 轉型脫離化石燃料、28(h) 逐步淘汰無效率化石燃料補貼，' +
        '皆為決議承諾；核能僅見於 28(e)「加速發展技術」清單、無「增加核能比例」目標，故唯一不在決議者為「增加核能比例」。' +
        '另收緊選項 A（原「逐步淘汰低效石化能源」非決議逐字條款）與選項 B（原「增加 2 倍」倍數口語歧義）措辭以確保 C 為唯一答案，' +
        '見 metadata.options_replaced。COP28 議定成果為史實、不隨時間變',
    },
    {
      id: 'gist[102]',
      why:
        '答案 B→A（2026-07-15，依 Reg (EU) 2023/1773 過渡期規則(a)：同一生產線先後產出→依使用時間分攤）。' +
        '2026-07-28 題幹明確限定為 CBAM 過渡期（2023-10-01～2025-12-31）歷史題並補「先後（不同批次）」條件，' +
        '使「使用時間」成為唯一正解（回應 issue #102 的先後/共同產出歧義）。過渡期已結束、該方法為固定歷史、不隨時間變 ' +
        '（正式期 2026+ 分攤方法已改由 Reg (EU) 2025/2547 之功能單位／物理關係規範，見 S_GAP-cbam-* 兩題）',
    },
    {
      id: 'S_CHU_07-q030',
      why:
    ...
    { id: 'S_CHU_06-q094', why: 'ISO 14064-1 對 base year 的定義 —— 定義，不隨時間變' },
    {
      ...
      why:
        ...
        '版本寫死的標準條文，不隨時間變：新版發布也不會改變「2018 版條文寫了什麼」。' +
        '（曾短暫標成 time_sensitive 以追蹤 ISO/WD 14067.2，但季排程只 curl URL 存活、' +
        '不查版本狀態也不比對條文，那等於宣稱一套不存在的監控，已撤回。）' +
        '原答案「準確度」不在清單裡（清單裡的是「精密度」）',
    },
    {
      id: 'gist[529]',
      why:
