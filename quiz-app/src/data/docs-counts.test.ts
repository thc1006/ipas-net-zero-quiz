...
describe('gate 缺口：README / DATA-PROVENANCE 的每一個數字都要有人守', () => {
  ...
  // 補解析的過程證明了 41 題的「已驗證引文」其實什麼也沒證明 ——
  // 多數只是把題目原樣抄一遍、沒抓到答案卡。這個數字要釘住，不准偷偷變小。
  it('README 的「41 題引文撐不住答案」必須等於資料裡的 evidence_review 標記', () => {
    const flagged = ALL.filter(
      (i) => (i.metadata as unknown as { evidence_review?: unknown })?.evidence_review != null
    ).length;
    const m = README.match(/另外 (\d+) 題 —— 它們的「已驗證引文」其實什麼也沒證明/);
    expect(m, 'README 找不到「另外 N 題 —— 它們的「已驗證引文」其實什麼也沒證明」').not.toBeNull();
    expect(Number(m![1])).toBe(flagged);
