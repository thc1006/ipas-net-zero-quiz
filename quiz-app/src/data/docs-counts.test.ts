...
describe('gate 缺口：README / DATA-PROVENANCE 的每一個數字都要有人守', () => {
  ...
  // NEEDS-SOURCING.md：交還給人工處理的「沒有可驗證來源」的題目清單。
  // 這個清單必須跟資料一致 —— 不能有題目被悄悄拿掉、或悄悄補進而不更新清單。
  it('NEEDS-SOURCING.md 列出的題目數，必須等於資料裡「無解析且無有效來源」的題數', () => {
    const needs = read('NEEDS-SOURCING.md');
    // 資料側：沒有解析、沒有 evidence、且（完全沒 source 或 source 被標記為無效）
    const noValidSource = ALL.filter((it) => {
    ...
    // 文件側：每題以「### <id>」列出
    const listed = [...needs.matchAll(/^### (\S+)/gm)].map((m) => m[1]);
    const missing = noValidSource.filter((id) => !listed.includes(id));
    const extra = listed.filter((id) => !noValidSource.includes(id));
    expect(missing, 'NEEDS-SOURCING.md 漏了這些無來源題').toEqual([]);
    expect(extra, 'NEEDS-SOURCING.md 多列了這些已有來源的題').toEqual([]);
  });
});
