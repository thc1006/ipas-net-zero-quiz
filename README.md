...
- **有來源的題目，來源是查得到的** —— 主題庫 **380 / 773 題**（49%）在 `metadata.sources`
  附一手來源 URL（環境部、EUR-Lex、IPCC、ISO、法規資料庫等）。
  這些 URL 由季排程 workflow 每季自動檢查是否還通。
  ⚠️ **其餘 393 題沒有來源連結** —— 它們來自社群共筆的考古題整理。
- **「沒有來源」不等於「是錯的」—— 我們去量了** —— 從那 393 題（原 422 題）隨機分層抽 **40 題**，
  逐題找一手來源並**用程式把引文抓回原網頁逐字比對**：
  **29 題引文驗證通過，錯誤 0 題**（8 題找不到一手來源 —— 付費標準／純算術／概念題）。
  點估計錯誤率 **0%**，95% 信賴上界約 **10%**（rule of three）。那 29 題的來源與逐字引文
  已寫回 `metadata.evidence`。
  ⚠️ 同一批蒐證中，**AI 代理捏造了 1 筆引文、引了 2 筆非一手來源** ——
  **全部被機械逐字比對擋下，沒有寫進題庫**。
  這就是為什麼**代理的「判定」不算數，只有「證據」算數**。
...
題庫中有 **122 題**的答案會隨法規變動（CBAM、碳費、NDC、碳中和標準）。
...
本輪只實查 **110 / 773** 題。判斷單一題目請看該題的 `metadata.valid_as_of`。
