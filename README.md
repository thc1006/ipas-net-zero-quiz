...
- **「有來源」分成三個等級 —— 不要混為一談**
  ...
  | 等級 | 意思 | 主題庫 | 練習池 |
  | --- | --- | ---: | ---: |
  | **① 有逐字引文，且經機械驗證** | 我們把來源網頁抓回來，**逐字確認那句話真的在上面** | **102 / 773** | **34 / 154** |
  | ② 有一手來源 URL | 附了法規資料庫／環境部／IPCC／ISO 等的連結（季排程每季檢查是否還通） | 372 / 773 | 144 / 154 |
  | ③ 完全沒有來源 | 來自社群共筆的考古題整理，**無從查證** | 393 / 773 | 0 / 154 |
...
- **代理的「判定」不算數，只有「證據」算數** —— 兩輪共派 19 個 AI 代理蒐證，
  契約寫死：**只准交「一手來源 URL + 該頁面上的逐字引文」，找不到就回報找不到，禁止用「我知道」填空**。
  每一筆引文都由程式抓回原網頁逐字比對。結果：
  - 第一輪 32 筆引文中，**代理捏造了 1 筆、引了 2 筆非一手來源** —— 全部被擋下，一筆都沒寫進題庫。
  - 第二輪把這件事寫進契約後，**133 題、0 筆捏造**。
  - 這兩輪找到 **4 題答案是錯的**（其中 `gist[335]`：IFRS S1 的揭露範圍是「**所有**永續議題」，
    不是氣候專屬 —— 氣候專屬的是 IFRS S2）。
- **重建的題目有證據鏈** —— 159 題逐題記錄來自哪一份 PDF（含 sha256）的哪一頁、哪一欄、
  ...
  可完整重現（實測 **159/159** 相符）
- **改過的答案留得下痕跡** —— 16 題答案曾被更正，每題都保留 `metadata.prior_answer`
...
題庫中有 **123 題**的答案會隨法規變動（CBAM、碳費、NDC、碳中和標準）。
