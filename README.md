...
- **「有來源」分成三個等級 —— 不要混為一談**
  ...
  | 等級 | 意思 | 主題庫 | 練習池 |
  | --- | --- | ---: | ---: |
  | **① 有逐字引文，且經機械驗證** | 我們把來源網頁抓回來，**逐字確認那句話真的在上面** | **703 / 773** | **110 / 154** |
  | ② 有一手來源 URL | 附了法規資料庫／環境部／IPCC／ISO 等的連結。⚠️ **「連結還通」不代表「指對地方」** —— 見下方引用複驗 | 740 / 773 | 149 / 154 |
...
- **「有一手來源 URL」曾經是一個很弱的訊號 —— 我們把每一個都打開來看了**
  ...
  | 複驗結果 | 題數 | |
  | --- | ---: | --- |
  | ✅ 引用正確（那一頁上真的有支持答案的原文） | **294** | 77% |
  | 🔧 **引錯地方 —— 那一頁根本不含這個主張** | **31** | **8.1%** |
  ...
  **這些 URL 全部都回 HTTP 200。連結健康檢查一個都抓不到。**
  ...
  處置：**19 題**已換成經機械驗證的一手來源（新來源上的引文逐字驗過）。
  另外 **12 題**代理說引錯、卻提不出可驗證的替代來源 —— **我們沒有動它**，
  只標記 `citation_audit.verdict = citation_disputed`。
...
本輪只實查 **111 / 773** 題。判斷單一題目請看該題的 `metadata.valid_as_of`。
