...
- **⚠️ 先講最重要的一件事：「引用被驗過」不等於「答案是對的」**
  ...
  **第一個能機械驗證「答案」而非「引用」的檢查**：
  ...
  > **120 題的答案已與官方答案卡逐題對過，0 題不符**（`tools/answer_key_crosscheck.py`）。
  > 這 120 題的答案被**釘死在資料裡**：任何人改動它們，CI 就紅。

  ⚠️ **比對必須按「文字」，不能按「字母」** —— 題庫把選項順序打散、文字也改寫過。
  我親手踩過：教材答案卡印 `(D)`、題庫標 `(A)`，我差點宣告那是錯答案 ——
  但教材的 (D) 就是題庫的 (A)。**字母是排版，文字才是內容。**

  ⚠️ **而全庫 927 題裡，只有這 120 題（12.9%）的答案被獨立驗證過。**
  剩下 807 題靠的仍然是「引用看起來對」。**我們不假裝那是同一回事。**

- **「有來源」分成三個等級 —— 不要混為一談**

  | 等級 | 意思 | 主題庫 | 練習池 |
  | --- | --- | ---: | ---: |
  | **① 有逐字引文，且經機械驗證** | 我們把來源網頁抓回來，**逐字確認那句話真的在上面** | **718 / 773** | **112 / 154** |
...
- **「有一手來源 URL」曾經是一個很弱的訊號 —— 我們把每一個都打開來看了**
  ...
  | 複驗結果 | 題數 | |
  | --- | ---: | --- |
  | ✅ 引用正確（那一頁上真的有支持答案的原文） | **303** | 79% |
  | 🔧 **引錯地方 —— 那一頁根本不含這個主張** | **28** | **7.3%** |
  | 📝 主題相關，但**沒有一句話釘得住這個答案** | 35 | 9.1% |
...
- **重建的題目有證據鏈** —— 159 題逐題記錄來自哪一份 PDF（含 sha256）的哪一頁、哪一欄、
  ...
  可完整重現（實測 **159/159** 相符）
- **改過的答案留得下痕跡** —— 32 題答案曾被更正，每題都保留 `metadata.prior_answer`
  與 `_correction_note`（改了什麼、憑什麼改）。其中 32 題附一手來源 URL，
