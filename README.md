...
- **「有一手來源 URL」曾經是一個很弱的訊號 —— 我們把每一個都打開來看了**
  ...
  **這些 URL 全部都回 HTTP 200。連結健康檢查一個都抓不到。**
  ...
  處置：**27 題**已換成經機械驗證的一手來源（新來源上的引文逐字驗過）；
...
- **重建的題目有證據鏈** —— 159 題逐題記錄來自哪一份 PDF（含 sha256）的哪一頁、哪一欄、
  ...
  可完整重現（實測 **159/159** 相符）
- **改過的答案留得下痕跡** —— 31 題答案曾被更正，每題都保留 `metadata.prior_answer`
  與 `_correction_note`（改了什麼、憑什麼改）。其中 31 題附一手來源 URL，
