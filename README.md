...
- **「有來源」分成三個等級 —— 不要混為一談**
  ...
  | 等級 | 意思 | 主題庫 | 練習池 |
  | --- | --- | ---: | ---: |
  | **① 有逐字引文，且經機械驗證** | 我們把來源網頁抓回來，**逐字確認那句話真的在上面** | **228 / 773** | **42 / 154** |
  | ② 有一手來源 URL | 附了法規資料庫／環境部／IPCC／ISO 等的連結（季排程每季檢查是否還通） | 495 / 773 | 149 / 154 |
  | ③ 完全沒有來源 | 來自社群共筆的考古題整理，**無從查證** | 277 / 773 | 0 / 154 |
...
- **代理的「判定」不算數，只有「證據」算數** —— 三輪共派 40+ 個 AI 代理蒐證，
  ...
  - 第三輪 143 筆引文中，**又抓到 6 筆捏造、3 筆拿 Google Drive 當「一手來源」** —— 同樣一筆都沒寫進去。
    **捏造率不會因為契約寫得更清楚就歸零。所以擋它的不能是契約，只能是程式。**
  - 找到 **8 題答案是錯的**、**2 題題目本身壞掉**（見下）。
  - ⚠️ **有一題是代理對、我錯。** `S_VOCUS_03-q004` 問 ISO 14067 的「數據品質」特性，
...
- **有兩題不是答案錯，是「題目本身壞掉」** —— 社群共筆抄題時抄壞了，而且**改答案救不了**：
  - `gist[14]`：保留了 iPAS 官方題幹（「何者**非**屬臺灣 2050 淨零路徑的**治理基礎**？」），
    四個選項卻全被換成自然碳匯／綠色金融／碳捕捉封存／淨零綠生活 ——
    這四項**全部都是「十二項關鍵戰略」**，沒有一項是治理基礎（治理基礎只有科技研發、氣候法制兩項）。
    於是這題**四個選項都是正確答案**。已依公版教材範例題 Q47 還原官方選項與答案卡，
    原選項留在 `metadata.options_replaced`。
  - `gist[61]`：公版教材明示「國家」不屬碳中和主體範圍，於是「國家」與「個人生活方式」
    **都**不屬於 —— 兩個正解。**改成任何單一字母都是猜的**，因此改為 `answer: null`
    並標記 `ambiguous`，**排除計分**（理由記在 `metadata.answer_ambiguity_note`）。
...
- **重建的題目有證據鏈** —— 159 題逐題記錄來自哪一份 PDF（含 sha256）的哪一頁、哪一欄、
  ...
  可完整重現（實測 **159/159** 相符）
- **改過的答案留得下痕跡** —— 23 題答案曾被更正，每題都保留 `metadata.prior_answer`
  與 `_correction_note`（改了什麼、憑什麼改）。其中 21 題附一手來源 URL，
  另外 2 題的依據是標準條文／公版教材本身（見該題的 `_correction_note`）。
...
題庫中有 **126 題**的答案會隨法規變動（CBAM、碳費、NDC、碳中和標準）。
