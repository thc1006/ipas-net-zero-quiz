...
- **「有來源」分成三個等級 —— 不要混為一談**
  ...
  | 等級 | 意思 | 主題庫 | 練習池 |
  | --- | --- | ---: | ---: |
  | **① 有逐字引文，且經機械驗證** | 我們把來源網頁抓回來，**逐字確認那句話真的在上面** | **100 / 773** | **29 / 154** |
  | ② 有一手來源 URL | 附了法規資料庫／環境部／IPCC／ISO 等的連結（季排程每季檢查是否還通） | 370 / 773 | 139 / 154 |
...
- **代理的「判定」不算數，只有「證據」算數** —— 兩輪共派 19 個 AI 代理蒐證，
  ...
  - 這兩輪找到 **3 題答案是錯的**（其中 `gist[335]`：IFRS S1 的揭露範圍是「**所有**永續議題」，
