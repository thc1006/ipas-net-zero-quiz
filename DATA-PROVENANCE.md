...
> 這個檢查**只驗網址還通不通，驗不出內容變了**。CBAM 憑證繳交期限從 5/31 改成 9/30、
...
## 命題規格：不得跨 taxonomy 拼接題幹與答案

issue #115 與 #116 是同一種病的兩個樣本。**兩則回報都主張「答案錯」，兩則都不成立 ——
但兩題確實都該修。** 這種缺陷比「答案字母錯」更深，因為每個零件單獨看都是對的：

- **#115**：題幹問 CBAM「哪類排放不計入」，回報者用 GHG Protocol 的範疇一/二/三來推論。
  兩邊各自都沒講錯，但 **CBAM 的內含排放量有自己的系統邊界，跟企業盤查的範疇分類是兩套 taxonomy**，
  不能互相代換。原題把兩套混在同一組選項裡，所以怎麼答都有話講。
- **#116**：ISO 14067 同時存在「5.10 準確度」（第 5 節的**原則**）與「6.3.5 精密度」
  （**數據品質特性**）。題幹只寫「ISO 14067 要求數據品質包含」，沒有標條款，
  於是兩個條款的詞彙被放進同一題互相競爭。

### 六條規格

1. **題幹先鎖制度／版本／條款／範圍。** 不要寫「依 CBAM」，要寫「依 CBAM 正式期排放計算施行規則
   Regulation (EU) 2025/2547 的 B.1」；不要寫「ISO 14067 要求」，要寫「依 ISO 14067:2018 第 6.3.5」。
2. **一題只能考一套 taxonomy。** 選項最好全部出自同一條款、同一層級。
   #115 改寫後四個選項全部來自 B.1 的 (a)(b)(c)，就沒有縫可以鑽。
3. **正解必須能由一段一手原文直接推出**，而不是來源只「跟主題有關」。
4. **每個誘答都要在同一 scope 下被否證。** 不能正解引正式期、誘答卻引過渡期；
   不能正解引 6.3.5、誘答卻引 5.10。否則考的不是知識，是猜命題者當時想的是哪一段。
5. **正式術語不得用近義詞代換。** precision 是「精密度」、accuracy 是「準確度」。
   「精確性」看起來差不多，但考試工具的用途正是幫人記住正式用語。
   標了 `metadata.canonical_term_zh_TW` 的題目，正解選項文字必須與其逐字相同（有 gate 擋）。
6. **條件式規則要寫出條件。** 不要寫「間接排放不納入 CBAM」——
   水泥、肥料須申報直接加間接，鋼、鋁、氫僅申報直接。把部分商品的規則寫成全制度通則，
   就是製造第二個可辯護的答案。

### 反例測試（命題完成後對每個誘答問一次）

> 有沒有某個商品、組織邊界、法規版本或條款，會讓這個誘答也成立？

#115 的「間接電力排放」就是在這一步被抓出來的：對鋼、鋁、氫只計直接排放，
所以那個誘答在特定商品下**其實是對的**。這也正是回報者的疑慮所在 —— 他指到了真缺陷，
只是給錯了理由。

### 對應的 gate

`quiz-app/src/data/dataset-integrity.test.ts`：
- `gist[64]` 必須以正式期法源 2025/2547 為來源，正解不得再是泛稱的「移動源排放」。
- `S_VOCUS_03-q004` 正解必須是「精密度」，選項不得再出現「精確性」或「準確性」。
- 通用：凡標了 `canonical_term_zh_TW`，正解選項文字必須逐字相同 ——
  擋住「evidence 寫精密度、選項寫精確性、測試照樣全綠」。
...
這個 workflow 需要在 **Settings → Actions → General → Workflow permissions**
